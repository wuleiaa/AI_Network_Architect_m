...
from utils.db_helper import get_db_path, init_db  # 仅此一处导�?
...
if "weekly_progress_count" not in st.session_state:
    st.session_state.weekly_progress_count = 0  # 初始�?0
...
if "s3_active_history_index" not in st.session_state:
    st.session_state.s3_active_history_index = None  # S3 当前查看的历史索�?
...
if "inquiry_active_history_index" not in st.session_state:
    st.session_state.inquiry_active_history_index = None  # 深度追问当前查看的历史索�?
...
with st.sidebar:
    ...
    if st.session_state.get("debug_save"):
        with st.expander("🔍 保存调试日志"):
            for log in st.session_state.debug_save[-5:]:  # 显示最�?�?
                st.text(log)
#===================================================================
    try:
    ...
    # 删除模式按钮
    if st.button("🗑�?删除模式", use_container_width=True):
    ...
    st.write(f"**当前状�?(已完�?{current_count}/10 任务)**")
    ...
    if current_count >= 10:
        ...
        # 新增这行：给学生一点动�?
        st.caption(f"加油！再完成 {10 - current_count} 个任务即可达成目�?🚀")
...
if "user_id" not in st.session_state:
    ...
    with col2:
        with st.form("login_form"):
            st.text_input("👤 用户�?, key="login_username")
        ...
        # 注册�?
        st.markdown("---")
        if st.button("�?没有账号？立即注�?, use_container_width=True):
            st.session_state.show_register = True

        if st.session_state.get("show_register", False):
            with st.form("reg_form"):
                ru = st.text_input("新用户名", key="reg_user")
                rp = st.text_input("新密�?, type="password", key="reg_pass")
    ...
    st.stop()  # ⚠️ 关键：阻止后续功能模块渲�?
# =============== 登录界面结束 ===============

# ==================== 模块一：智能故障诊疗室 (S1 升级�? ====================
if menu == "💻 代码智能诊疗":
    st.header("💻 单片机代码智能诊�?| AI导师引擎")
    st.markdown("LED 不亮？定时器不准？别急着问答案，先告诉我**你觉�?*哪里出了问题�?)
    ...
    with col1:
        ...
        topic = st.selectbox(
            "🔬 实验主题",
            [
                "GPIO 控制LED", "定时器中�?, "UART 串口通信", "I2C 总线通信", "SPI 通信协议",
                "ADC 电压采集", "PWM 电机调�?, "外部中断配置", "看门狗定时器", "DMA 数据传输",
                "按键消抖处理", "LCD 液晶显示", "DS18B20 温度采集", "EEPROM 读写操作", "RTC 实时时钟",
                "CAN 总线通信", "红外遥控解码", "步进电机驱动", "超声波测�?, "DAC 波形输出"
            ],
            index=0
        )
        ...
        user_thought = st.text_area(
            "🤔 我的初步排查思路 (必填)",
            height=100,
            placeholder="例如：我觉得�?GPIO 初始化模式没配对，或者是中断优先级设置有问题..."
        )
        ...
        analyze_btn = st.button("提交�?AI 导师", use_container_width=True)
    ...
    with col2:
        ...
        if analyze_btn:
            if not user_thought:
                st.warning("⚠️ 请先填写你的排查思路！学习不能只�?AI�?)
            elif not user_code:
                ...
                with result_box:
                    with st.chat_message("assistant", avatar="🤖"):
                        ...
                        # --- 关键修改：st.write_stream 会返回完整的字符�?---
                        # 我们不仅把它打印出来，还顺手存进 session_state �?
        ...
        # 场景 C (新增)：默认初始化状�?
            ...
            with result_box:
                with st.chat_message("assistant", avatar="🤖"):
                    ...
                    我会在这里为你提�?**苏格拉底式诊断引�?*，助你找到问题根源！🛠�?
                    """)


# ==================== 模块二：今日定制靶场 (S3 升级�? ====================
elif menu == "🎯 实验靶场工坊":
    st.header("🎯 实验靶场工坊 | 单片机实训生�?)
    ...
    with c3:
        st.write("")
        st.write("")
        gen_btn = st.button("生成任务�?, type="primary")

    st.markdown("---")

    # --- 核心逻辑修改：引�?Session State 防止刷新丢失 ---

    # 1. 负责生成任务 (当点击生成按钮时)
    if gen_btn and today_focus:
        # 重置答案显示的开关，因为生成了新�?
        st.session_state.s3_show_answer = False
        st.session_state.current_task_scored = False  # <--- 新增这行：重置计分状态，允许新任务加�?
        ...
        st.session_state.s3_task_submitted = False
        st.session_state.s3_is_correct = False
        with st.spinner(f"正在构建关于【{today_focus}】的单片机实验方�?.."):
            # 调用 AI 生成任务
            stream = st.session_state.ai_engine.generate_personalized_task(today_focus, level)
            # 关键点：st.write_stream 会返回完整的生成文本，我们将它存�?session_state
            # 这样点击"查看答案"刷新页面后，题目文字才不会消�?
    ...
    # 2. 负责显示任务 (只要 session 里有题目，就一直显示，不管是刚生成的还是刷新后�?
    ...
    # 3. (新增) 默认初始化状�?- 显示引导�?
        ...
        with st.chat_message("assistant", avatar="🤖"):
            st.markdown("""
                ...
                AI 导师将根据你的选择，量身定�?**独一无二** 的实战编程任务与硬件调试挑战！�?
                """)
...
if ("s3_task_text" in st.session_state and st.session_state.s3_task_text) and st.session_state.s3_active_history_index is None:
    ...
    with sub_col1:
        ...
        if submit_btn and student_code.strip():
            ...
            # 检测AI是否认为答案已正�?
            if "你的方案已完全正�? in feedback_text or "已完全正�? in feedback_text:
    ...
    with sub_col2:
        ...
        if len(st.session_state.s3_tutor_history) > 0:
            st.caption("已迭�?{} �?.format(st.session_state.s3_tutor_round))
            for i, msg in enumerate(st.session_state.s3_tutor_history):
                ...
                with st.expander("{} {} - 第{}�?.format(role_icon, role_name, (i//2)+1), expanded=(i == len(st.session_state.s3_tutor_history)-1)):
    ...
    # 当没有点击按钮，且没有历史记录时，显示一个白色的空回答框（引导框�?
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            st.markdown("""
                ...
                请在上方输入你想要深入理解的单片机概念（�?GPIO模式、定时器原理、中断响应流程等），
